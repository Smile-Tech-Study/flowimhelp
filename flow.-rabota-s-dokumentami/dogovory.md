...
Заявка сдающего "цепляется" к конкретным двум договорам (Сублицензионному и Лицензионному) в момент, когда Flow получает из Odin статус экзамена. Т.е. после того, человек реально сдавал экзамен в Odin, не зависимо от результата - сдал или не сдал. Те, которые не явились на экзамен, в квоте не учитывают.\
Сравнить количество заявок, которые должны быть в квоте можно, например, по дашборду [  ](https://datalens.yandex.cloud/7zasrezsdg6wu-formirovanie-schyota?tab=wK)<https://datalens.yandex.cloud/7zasrezsdg6wu-formirovanie-schyota?tab=wK>
...
1. После того, как заявка "прицепилась" к договорам, эти договоры должны отобразиться на странице заявки, на подобии, как это сделано в Содействии занятости:\
   \
   Строки будут звучать так:\
   Номер и дата лицензионного договора: …\
   Номер и дата сублицензионного договора: …

   Номера должны быть кликабельными ссылками на страницы этих договоров.

   Договоры должны отображаться после строки со стоимостью:\
   \
   Строки с договорами на странице заявки должны отображаться ролям: Администратор, Сотрудник центра тестирования, Сотрудник экзаменационной площадки.

2. У роли Администратор возле строк с номерами договоров должна быть иконка "Карандаш".\
   Администратор должен иметь возможность вручную поменять договор, к которому прицеплена заявка.\
   При клике на карандаш должно появляться окно с выпадающим списком всех договоров конкретного типа (лицензионный или сублицензионный) и данной конкретной организации. В выпадающем списке должна быть строка поиска по номеру.\
   Под выпадающим списком должна быть кнопка "Сохранить".

3. В списке заявок (в новом и пока старом) должны быть добавлены два столбца: Лицензионный договор, Сублицензионный договор. По этим столбцам должен быть фильтр и сортировка.\
   Столбцы должны быть скрытыми по умолчанию.\
   Столбцы должны попадать в выгрузку Excel, если они выбраны.

4. Если во Flow у заявки был удален статус завершения экзамена - значит заявка должна "отцепиться" от обоих договоров. Поля с договорами должны очиститься.

5. Запись в квоту конкретного договора конкретной заявки и Удаление из квоты конкретного договора конкретной заявки должны быть зафиксированы в истории заявки.

6. Остаток квоты по сдающим должен сокращаться в момент, когда Flow получает из Odin статус экзамена.\
   Остаток квоты по сдающим = Квота по сдающим - Количество "прицепленных" к договору заявок

7. Остаток квоты по сумме зависит от остатка квоты по сдающим и от типа договора.\
...
8. Если квота по сумме перерасходована, например, заявка "насильно" прицеплена к договору администратором (п.6), то могут быть отрицательные числа в остатках.

9. Примерное число дней для расхода остатка квоты = Остаток квоты по сдающим / Среднее количество заявок, цепляемых к договору ежедневно за последний месяц